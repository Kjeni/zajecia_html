 ...
 */
 ...
 function PrzegladarkaZdjec(id, zdjecia)
 {
     ...
     {
         html += '<option value="' + zdjecia[i][0] + '"' + (i == nr ? ' selected' : '') + '>' + zdjecia[i][1] + '</option>';
     }
     
     document.write(
         '<form action="javascript:void(0)" id="' + this.id + '" class="zdjecia">' +
             '<div class="zdjecia_menu">' +
                 '<input type="button" name="poczatek" value="|&lt;&lt;"' + (nr == 0 ? ' disabled' : '') + ' title="Początek" onclick="' + this.id + '.wyswietl(0)" onkeypress="' + this.id + '.wyswietl(0)">' +
                 '<input type="button" name="wstecz" value="  &lt;&lt;  "' + (nr == 0 ? ' disabled' : '') + ' title="Wstecz" onclick="' + this.id + '.wyswietl(this.form.elements[\'zdjecia\'].selectedIndex - 1)" onkeypress="' + this.id + '.wyswietl(this.form.elements[\'zdjecia\'].selectedIndex - 1)">' +
                 ' <select name="zdjecia" onchange="' + this.id + '.wyswietl(this.selectedIndex)">' +
                     html +
                 '</select> ' +
                 '<input type="button" name="dalej" value="  &gt;&gt;  "' + (nr == zdjecia.length - 1 ? ' disabled' : '') + ' title="Dalej" onclick="' + this.id + '.wyswietl(this.form.elements[\'zdjecia\'].selectedIndex + 1)" onkeypress="' + this.id + '.wyswietl(this.form.elements[\'zdjecia\'].selectedIndex + 1)">' +
                 '<input type="button" name="koniec" value="&gt;&gt;|"' + (nr == zdjecia.length - 1 ? ' disabled' : '') + ' title="Koniec" onclick="' + this.id + '.wyswietl(this.form.elements[\'zdjecia\'].options.length - 1)" onkeypress="' + this.id + '.wyswietl(this.form.elements[\'zdjecia\'].options.length - 1)">' +
                 '<div><input type="checkbox" name="auto"' + (auto ? ' checked' : '') + ' id="' + this.id + '__auto" onclick="' + this.id + '.przewin()" onkeypress="' + this.id + '.przewin()" style="vertical-align: middle"> <label for="' + this.id + '__auto">Pokaz slajdów</label></div>' +
             '</div>' +
